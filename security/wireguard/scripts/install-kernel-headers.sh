...
if apt-cache show -q linux-headers-$(uname -r) > /dev/null 2>&1 /dev/null; then
  ...
  sudo apt-get -yq install linux-headers-$(uname -r)
  exit $?
fi
...
sudo apt-get install -y libssl-dev # adapt to your package manager
sudo make -C /usr/src/linux prepare modules_prepare